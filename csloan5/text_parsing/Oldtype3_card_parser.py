...
def parseText(lines, card):
    ...
    for count, line in enumerate(lines):
        ...
        # Make sure to only extract the first match for the section quarter
        if not found_Qtr_Qtr:
            # Extract the Section quarter of the well
            Qtr_Qtr = re.search('(C\ |C\/2\ )?((((N|E|S|W)\/2\ ?)|(NE|NW|SE|SW))\ ?){2,3}C?', line)
            if Qtr_Qtr:
                card.Qtr_Qtr = re.split('(SEC|C)$',Qtr_Qtr.group())[0]
                found_Qtr_Qtr = True
        ...
        if spud:
            temp = re.split('COMM\.?\ ?', spud.group())[-1]
            card.spud = temp.replace('-', '/')

        # Extract the completion date
        comp = re.search('COMP\.?\ ?\d{1,2}(\/|\-)\d{1,2}(\/|\-)\d{2,4}', line)
        if comp:
            temp = re.split('COMP\.?\ ?', comp.group())[-1]
            card.completion = temp.replace('-', '/')
        ...
        casing = re.search('[0-9]{1,2}"\ ?([0-9]\/[0-9])?\ ?[0-9]{2}', line)
        if casing:
