""" TODO:
    ...
    - Somehow figure out how to extract DSTs and core info --- DONE
...
def parse_lines(df, parsed):
    for line in df.itertuples(index=False):
        ...
        # NS SW Footage
        if re.search('\(?[0-9]{1,4}\ ?(n\/s|fnl|s\/n)\ ?[0-9]{1,4}\ ?(e\/w|fwl|w\/e)\)?', text):
            parsed['NSFootage'] = re.search('\(?[0-9]{1,4}\ ?(n\/s|fnl|s\/n)\ ?', text).group(0)
            parsed['EWFootage'] = re.split('\(?[0-9]{1,4}\ ?(n\/s|fnl|s\/n)\ ?', text)[-1]
        ...
        if re.search('(COMP|COMPL):?\ ?[0-9]{1,2}(\/|-)[0-9]{1,2}(\/|-)[0-9]{2,4}', text, re.IGNORECASE):
            parsed['CompDate'] = re.split('(COMPL|COMP):?\ ?', text, re.IGNORECASE)[-1]
            
        # Elevation
        if re.search('(ELEV|El):?\ ?[0-9]{4}.*', text, re.IGNORECASE):
            parsed['Elevation'] = re.split('(ELEV|El):?\ ?', text, re.IGNORECASE)[-1]
            
        # API number
        if re.search('(API|IDN)?:?\ ?49[-~]?[0-9]{3}[-~]?[0-9]{5}', text, re.IGNORECASE):
...
def main():
    ...
    for index, file in enumerate(filenames):
        ...
        csv['text'] = csv['text'].replace("['~`!=_]", "", regex=True)
        csv['text'] = csv['text'].replace(";", ":", regex=True)
        
        Formations = ""
        DSTs_and_Cores = ""
        # Grab DSTS and Cores Text section before realigning.
        for line in csv.itertuples(index=False):
